...
local function uP()
    ...
    if cCF and sP then
        local mR = math.min(b, 80)
        for i = 1, mR do
            local step = (i / mR) * (2 * math.pi)
            local p = Instance.new("Part", pFld)
            p.Size = Vector3.new(0.05, y, sZ * (b / mR))
...
bD.MouseButton1Click:Connect(function()
    ...
    for i = 1, nB do
        ...
        if t1 then
            h:EquipTool(t1)
Use code with caution.local wZ = workspace:WaitForChild("WhiteZone")local rot = CFrame.new(-10, 6.1, -20) * CFrame.Angles(0, -a, 0)t1:WaitForChild("RF"):InvokeServer("PlasticBlock", 8001, wZ, rot, true, bP, false)rS = rS + 1endtask.wait(0.05)local pB = f:FindFirstChild("PlasticBlock")local t2 = P.Backpack:FindFirstChild("ScalingTool") or ch:FindFirstChild("ScalingTool")if pB and t2 thenh:EquipTool(t2)pB.Name = gIDt2:WaitForChild("RF"):InvokeServer(pB, Vector3.new(sX, sY, sZ), bP)rS = rS + 1endtask.wait(0.05)if uC thenlocal t3 = P.Backpack:FindFirstChild("PaintingTool") or ch:FindFirstChild("PaintingTool")if pB and t3 thenh:EquipTool(t3)t3:WaitForChild("RF"):InvokeServer({{{pB, cP.BackgroundColor3}}})rS = rS + 1endtask.wait(0.05)endh:UnequipTools()if rS >= 15 thentask.wait(0.5)rS = 0endendfor _, b in ipairs(f:GetChildren()) doif b.Name == gID thenb.Name = "PlasticBlock"endendend)uP()
