...
-- Replaced unstable Discord function with an accessible select-to-copy textbox frame
...
btnBuild.MouseButton1Click:Connect(function()
    ...
    for i = 1, steps do
        ...
        if dynamicBlockPath then
            -- Resolved optimization method exception crash by utilizing standard Vector structure
            local sVec = Vector3.new(sizeX, sizeY, sizeZ)
            sRF:InvokeServer(dynamicBlockPath, sVec, pCF)
            task.wait(0.01)
            
            -- Resolved remote tool array payload alignment crash to allow live painting mechanics
